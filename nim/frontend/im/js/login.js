var Login = {
	...
	*/
	...
	requestLogin: function(account, pwd) {
		...
		$.post(CONFIG.url + '/im/login',{
			account:account,
			pwd:pwd
		},function (res) {
			if(res&&res.infoNo === 1){
				setCookie('uid',res.data.nimAccount.accId);
				setCookie('sdktoken',res.data.nimAccount.token);
				window.location.href = './main.html';
			}else{
				that.$errorMsg.html("账户密码错误!").removeClass('hide');  // 显示错误信息
			}
		},'json');
